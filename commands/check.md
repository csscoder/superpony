...
When it returns: surface the verdict (APPROVE / APPROVE_WITH_FIXES / REVISE / BLOCK) and every finding. Apply the `receiving-code-review` discipline — verify each finding, push back on the wrong ones with reasoning, never hide them — then fix before the next step.
